...
\paragraph{Learning with L1 costs}
...
Mathematically, this averaging over spikes condition is formulated as the loss at time $t$

c
using the fact we can express the Voltage post-spike $\bmu{V}^{\text {post}}$ as
\begin{equation}
...
For $\beta = \frac{1}{2}$ no regulation is present since in the optimal case  $\bmu{W}^f$ will be twice the threshold Voltage.\\\todo{Keep this in?}
However values $\beta\approx\frac{1}{2}$ lead to unstable ping pong patterns described previously.\\
...
\paragraph{Learning with $L2$ costs}
...
\begin{equation}
\Delta \bmu{W}^f_{ij} =-\frac{\partial L_t}{\partial \bmu{W}^f_{ij}} \propto - \beta\left( \bmu{V}_i^{\text {pre}}\left(t\right) + \mu \bmu{r}_i(t)\right) -\bmu{W}^f_{ij} -\delta_{ij}
\end{equation}
when a spike is fired.
\todo{Throw this out. We skip the co¸mparison between learning methods.}
