...
The most important insight of the approach in \cref{eq:feeback_c,eq:feedback_c2} is that the signal $\bmu{c}(t)$ is the pricipal source of control in the system. Moreover it is explicitly calculated from the system $\bmu{A}$ as well as the information of the reference signal and its derivative.




\section{Learning of network parameters}
...
\subsubsection{Fast connection weights $\bmu{W}^f$}

\subsubsection{Slow connection weights $\bmu{W}^s$}
