...
Although the above mentioned problems, rate encoded \acp{SNN} have seen interest by research. A big hurdle of deploying \acp{SNN} is the lack of performant learning algorithms. For this there efforts have been made to train recurrent \acp{ANN} using backpropagation and afterwards convert the trained network to a \ac{SNN}.
\rewrite{Explain a bit more and make sure that the indeed use rate based encoding. Maybe find a reference that the power consumption is really low even though the use rate based encoding}
