...
\todo[inline]{here maybe a figure showing the comparison with and without the addition might be better?}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{../../plots/Control/compare_step_both methods.pdf}
...
\subsubsection{Suboptimal $\bmu{B}$ and $\bmu{C}$}
...
\todo[inline]{Prob rewrite this a bit that we are fine with this constraint. Also the exception with higher order systems (they seem to work just fine)}
\todo[inline]{also mention that this was observed with the error free approach making it a more interesting option}
\todo[inline]{the error version was a bit better performing when adding noise. Natural since the direct error term helps out in that regard}
...
Moreover, noise in the reference and its derivative can negatively impact performance\todo[inline]{Verify this. Give an example or a proof.}\\
...
\todo[inline]{this is messy...and incoherent}

All this supports the idea that the feed-forward inputs are the governing force in the network's performance. Furthermore by the removal of the trajectory and only the use of its derivative show more the characteristics of an open loop controller.\todo{Is that really so?}\\
...
The second big limitation comes with the choice of $\bmu{\Gamma}$. During testing, the choice of $\bmu{\Gamma}$ had immense impact on the network convergence. With randomly chosen $\bmu{\Gamma}$ on $\mathit{S}^n$ network convergence was not guaranteed anymore. Moreover, if convergence was reached results exhibited relative errors of more than 1000\% making them completely unusable. This was especially the case when working with higher dimensional systems. The necessary accuracy for $\bmu{W}^f$ to yield a functioning network was not learned anymore. Without sufficiently approximated $\bmu{W}^f$, training was worsening the networks performance.\\
It was therefore necessary to choose $\bmu{\Gamma}$  as equidistant points on $\mathit{S}^n$ to have uniform error projection and re convergence.\\
Apart from the choice of $\bmu{\Gamma}$, higher order problems pose a problem for the networks learning. Due to small inaccuracies in training, errors add up in higher dimensional systems. Especially $\bmu{W}^s$ becomes the major factor in network error.\\
However these errors become significant only when no or little external input $\bmu{c}(t)$ is applied. If $\bmu{c}(t)$ is zero for large parts of the simulation time, errors in $\bmu{W}^s$ compound to the point where the network produces unusably erroneous results. If $\bmu{c}(t)$ is non-zero, errors in $\bmu{W}^s$ become manageable and the network output acceptable.\\
...
%\begin{figure}
%	\centering
%	\includegraphics[width=\textwidth]{../../plots/Learning/full_jump_response.pdf}
%	\caption{Spike distribution for our model problem and $A = -10\bmu{I}$. Cumulative sum of all spikes for each epoch.}
%	\label{fig:full_comb_jump_response}
%\end{figure}

Unbedingt die noise testen
Verschiedene Referenz trajectories
Einmal performance mit 0 init
Vlt kann der controller das ja ausregeln wenn Ws und Wf scheisse sind. Wahrscheinlich schwiering wenn es kein feedback gibt. aber vlt decayed der fehler auch