...
\end{aligned}
\end{equation}
Now, let's consider a scenario where the network is falling short of the reference value, leading us to deduce that $e(t)>0$. Consequently, we can promptly conclude that the error term is conflicting with the intended definition of our voltage. This is achieved by inadvertently increasing the voltage of Neuron 1 in the wrong direction.\\
...
Can be fixed with an extra sign or using abs on the matrix

also i did not find a system where the error was significant in the performance

\paragraph{Example in paper is useless...}


\paragraph{Useless Output feedback???}

\paragraph{Adjustment for the Outputfeedback, even though it is useless}
