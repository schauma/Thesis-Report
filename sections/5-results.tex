...
\todo[inline]{It is possible to show a plot in case.}
...
\subsection{Combining the two networks}\label{par:combining-2-network}
...
To begin we set a sine wave as our target. As can be seen in \cref{fig:all_open_sine_base}, the aforementioned control using $\bmu{c}$ appears to deliver good results using the optimal weights. Both state trajectories almost perfectly overlap the target. As seen before, similar to \cref{par:combining-2-network}, dynamics quickly diverge due to the imperfections in the learned matrices making it impossible for the open loop to properly control the system, leading to an almost complete loss of the target trajectory. Yet the output shows better tracking than the separate networks.
\begin{figure}
	\centering
...
While we still send add the target trajectory to $\bmu{c}$ we can move the feedback loop directly into the network. This is done similarly like the matrices of fast and slow weights $\bmu{W}^f$, $\bmu{W}^s$ were introduced.\\
\todo[inline]{did we introduce this before?}
By using the definition of $\bmu{\hat{x}}$ we can formulate the loop as a matrix given by
\begin{equation}
	\bmu{W}^e = \bmu{\Gamma}^T\bmu{\Gamma}
\end{equation}
and add this matrix to the network dynamics as illustrated in \cref{fig:schematic_feedback_control}.
\begin{figure}
	\centering
...
\todo[inline]{We can skip that the loop and the extra matrix work identically....}
...
\end{equation}
It shows that in this test case the training was able to resolve the subtraction with relative errors between both matrices being less than 8\%.\todo[inline]{Idk if this is really correctly written. What I want to say is that after both trainings the subtraction is clearly visible with the off diagonals being off from zero less than 0.023 and the main diagonals being of from an identity matrix by at most 0.075 which is kind of impressive.}
...
\begin{equation}\label{eq:feedback_with_B}
	\bmu{c} = \bmu{BB}^+\left(\bmu{\dot{x}- Ax}\right)
\end{equation}
ignoring feedback. But $\bmu{B}^+$ is often unavailable and biologically it is unrealistic to assume that the brain does matrix inversion. Instead, we restrict ourselves to matrices $\bmu{B}$ such that
\begin{equation}
	\bmu{BB}^+ = \bmu{BB}^T,
\end{equation}
...
\todo[inline]{I guess this is more biologically plausible. but how much? Ask arvind}
...
\end{figure}
Unfortunately, the addition of $\bmu{B}$ ruins the network performance. This is mainly caused by inaccuracies in $\bmu{W}^s$, as the network shows perfect results in \cref{fig:all_closed_1_net_with_b} when both $\bmu{W}^s$ and $\bmu{W}^f$ (red) or just $\bmu{W}^s$(not shown) is computed analytically.\\


\subsubsection{Control with rand init matrices for learning}
Even with feedback, it was not possible to use the matrices generated with random initialization from \cref{sssec:rand_init} regardless of the aforementioned problems with the input matrix $\bmu{B}$. Due to the lack of convergence in $\bmu{W}^f$ the network control cannot compensate. Unfortunately the error in both matrices were too large to yield usable results.\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo[inline]{The limits below were from an earlier subsubsection. Build them together into one}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\todo[inline]{Start introducing the single network idea}
...
\todo[inline]{Accuracy of Ws is a limitation}
